...
A = int(10 * 2**2 * 10000)  # int(1000 * 2**2 * 10000)
gamma = int(1.3e-4 * 1e18)  # int(2.2e-6 * 1e18)
D = 2 * 10**18
N = 2
dp = 1e-8
dx = 1e-5
...
def f_profit_deriv(A, gamma):
    ...
    d_price = np.linspace(-8e-1, 8e-1, 6000)
...
if __name__ == '__main__':
    import pylab
    d_price = np.linspace(-5e-2, 5e-2, 1000)
    pylab.semilogy(d_price, f_liq_density(A, gamma)(d_price))
    pylab.show()
